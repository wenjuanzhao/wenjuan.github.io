...
Base.prototype.index=function () {
    ...
    for(var i=0;i<parentNode.children.length;i++){
		if(this.elements[0]==parentNode.children[i]){
			return i;
		}
	}
...
Base.prototype.center = function (width, height) {
	var top = (getInner().height - height) / 2+getScrollTop().top;
	var left = (getInner().width - width) / 2+getScrollTop().left;
	for (var i = 0; i < this.elements.length; i ++) {
...
Base.prototype.lock = function () {
	for (var i = 0; i < this.elements.length; i ++) {
		this.elements[i].style.width = getInner().width+getScrollTop().left + 'px';
		this.elements[i].style.height = getInner().height+getScrollTop().top + 'px';
		this.elements[i].style.display = 'block';
...
Base.prototype.resize = function (fn) {
	for (var i = 0; i < this.elements.length; i ++) {
		...
		addEvent(window, 'resize', function () {
			fn();
			if (element.offsetLeft > getInner().width+getScrollTop().left - element.offsetWidth) {
				element.style.left = getInner().width+getScrollTop().left - element.offsetWidth+ + 'px';
				if(element.offsetLeft<=0+getScrollTop().left){
					element.style.left = 0 +getScrollTop().left+ 'px';
				}
			}
			if (element.offsetTop > getInner().height+getScrollTop().top - element.offsetHeight) {
				element.style.top = getInner().height+getScrollTop().top - element.offsetHeight + 'px';
				if(element.offsetTop<=0+getScrollTop().top){
					element.style.top = 0+getScrollTop().top + 'px';
				}
			}
		});
	}
